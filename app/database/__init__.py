...
def update(first_name, last_name, hobbies, pk):
    cursor = get_db().execute(
        "UPDATE FROM user WHERE id=? "
    ), (first_name, last_name, hobbies, pk).lastrowid
    cursor.commit()
    cursor.close()
    return output_formatter(cursor)


def delete(first_name, last_name, hobbies, pk):
    cursor = get_db().execute(
        "DELETE FROM user SET first_name=? AND last_name=? AND hobbies=? WHERE id=?"
    ), (first_name, last_name, hobbies, pk).lastrowid
    cursor.commit()
    cursor.close()
    return output_formatter(cursor)